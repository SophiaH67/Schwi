# This is copied from the pypi package "amiami". The
# only change is this is async.
import httpx
...
class ResultSet:
    ...
    def add(self, productInfo):
        ...
        if availability == "Unknown status?":
            print(
                "STATUS ERROR FOR {}: flags:{}, avail:{}".format(
                    productInfo["gcode"],
                    flags,
                    availability,
                )
        ...
        item = Item(
            productURL="https://www.amiami.com/eng/detail/?gcode={}".format(
                productInfo["gcode"]
            ),
...
async def search(keywords):
    ...
    async with httpx.AsyncClient() as client:
        ...
        hasMore = True
        while hasMore:
            resp = await client.get(rootURL, params=data, headers=headers)
