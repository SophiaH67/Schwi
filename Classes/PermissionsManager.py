...
class PermissionsManager:
  ...
  async def get_permissions(self, user_id: int | str):
    user_id = str(user_id)
    permissions = self.schwi.redis.get(f"schwi:permissions:{user_id}")
    if permissions is None:
      await self.set_permissions(user_id, PermissionLevel.USER)
      return PermissionLevel.USER
    else:
      return permissions.decode("utf-8")

  async def set_permissions(self, user_id: int | str, level: str):
    user_id = str(user_id)
